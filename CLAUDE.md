...
- **`skills/*.py`** — one module per skill (`teste_fila.py`, `roteiro.py`, `video_explicativo.py`). All three are currently placeholders (`time.sleep()` + write a file to `files/`) that prove the pipeline works end-to-end; swapping in real generation only means changing the body of each `run()`.
