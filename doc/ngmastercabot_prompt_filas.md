# Prompt para implementar filas no `ngmastercabot`

## Missão

Você está trabalhando no projeto `ngmastercabot`.
...
→ o ngmastercabot envia o resultado ao usuário
```
...
O `ngmastercabot` é a porta de entrada.
...
```text
ngmastercabot.service
ngmastercabot-video-worker.service
ngmastercabot-text-worker.service
ngmastercabot-service-worker.service
ngmastercabot-watcher.service
```
...
O `ngmastercabot` continuará respondendo perguntas normais imediatamente.
